...
- Add omitted product sections, the contents of drawers/tabs, and any product-specific size chart used for a fit, length, measurement or variant-identity claim, with its measurement basis and supported model fit evidence. Do not fetch a competitor size chart solely for Ondine's default shared BODY guide. Record checked absences only after inspecting the source. Keep optional unknowns absent.
...
## Current composition, source options and product-specific fit facts
...
An explicit single UK numeric source label such as `UK 8 (S)` may display as `8` through `ondine_uk_numeric_size_identity_v1`. This preserves the supplier's stated UK number; it is not a conversion from S or a range. Retain the exact original source label in the FactPacket and use the numeric target in the Size option, size module and numeric SKU code. Other source labels retain their captured identity in the evidence. Letter-only, other-market, ambiguous or colliding labels remain unresolved under the current UK-numeric listing default; do not imply that preserving them in evidence makes the listing valid. The shared BODY guide does not convert them into UK numeric sizes.

Ondine's default shared BODY guide is not a FactPacket size mapping. It does not convert supplier labels, establish supplier equivalence, change source option identity or require a product-specific chart. Keep it separate from the variant-identity check above and from verified product-specific fit, length and measurement facts.
...
The Python desired state uses transport-neutral field names, not a ready-to-send GraphQL input. It retains the category path, GMC fields and verified weight when present. Through the existing connector, write the actual taxonomy category first, resolve the applicable category-metafield definitions and metaobject IDs for the connected Ondine shop, assign them, and verify both the live category and display values on read-back. Do not mistake a mock transport comparison or the offline shape check for that live category verification or the complete data-ready gate. Never invent a taxonomy/metaobject ID or a missing weight.

## Existing product-specific size-label exceptions

These records preserve an already explicit user decision. They are not a required step for a normal UK-numbered listing or shared-guide use, and this change does not authorize a new supplier-mapping process.
