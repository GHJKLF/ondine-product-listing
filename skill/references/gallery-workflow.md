...
### Source design details: capture → prompt → visual QA

Before rendering any slot, inspect source photos across all available views and record `product.design_details` in `product_reference_facts.json`. This required non-empty list describes identifying construction and trim, not a generic garment summary. Each entry has `feature`, `placement` and `source_evidence` strings. Record exact visible count per side, colour/material, attachment point and whether placement is bilateral, asymmetric or unknown. Use wearer-left/right; distinguish these from the image's left/right. Name the actual inspected source image path/URL and view in `source_evidence`. A hidden or cropped side is unknown, not absent. Inspect another available angle before concluding single-sided placement; resolve conflicting source evidence before generation.

For example, when verified in source images: `{"feature":"Three gold buttons on each shoulder (six total)","placement":"Bilateral: three on wearer-left shoulder and three on wearer-right shoulder","source_evidence":"<actual inspected source image path/URL and view>"}`. This example is not a default for other products. Capture other defining details the same way. If no decorative trim exists, record a verified construction feature instead; never fabricate a detail just to satisfy the renderer.

`render_slot_prompt.py` refuses missing or incomplete design-detail records and carries the exact list into every rendered slot, including the separate square. Include it in direct-tool prompts and every colour adaptation too. Check the garment description, detail focus and final submitted prompt for contradictions before generation. When a source correction supersedes an old one-sided assumption, correct the run facts and affected prompts first and invalidate affected prior QA; preserve the original attempts as history. An accepted/generated front is a continuity reference, never evidence that overrides source construction.

For each output, visually compare the actual source and output: count and placement on each visible side, symmetry/asymmetry, material/colour and attachment. Record per-detail results in run QA as match, mismatch or not visible, with the inspected source and output paths. Do not claim a hidden detail was verified. Front and ghost views must expose defining bilateral trim sufficiently to check both sides; adjust obstructing hair, pose or styling if necessary. A side/back/detail crop need only show what its angle supports, but it must not contradict the source. Reject omitted, extra, mirrored or relocated trim; regenerate only affected assets within the existing retry limits. Recheck additional colours and the GMC square independently. Text validation cannot establish visual fidelity.
...
After slot 01 and its square pass internal QA, generate and internally check `01b` using the second-model template, then generate slots 02–06 from their matching JSON files without a standalone second-model approval request. Compare slots 01 and 01b side by side before accepting 01b. They must immediately read as two different adult people; a changed pose, makeup, styling or crop alone fails. Regenerate 01b with an explicitly different identity if it reads as the lead model or a lookalike. Include the accepted second-model image in the completed draft gallery. Preserve garment, colour and visual system continuity. Reject any asset that fails its slot checks.
