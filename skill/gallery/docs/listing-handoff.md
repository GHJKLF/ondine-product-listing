...
The app works with images already uploaded to Shopify. The listing process remains responsible for generating photos with ChatGPT’s built-in image generation, performing internal image QA, uploading them to the correct draft product, and recording the resulting Shopify IDs. This app does not generate or upload photos and does not publish products.

The canonical product-listing reference calls this compiler after internally checked media upload and verification. The 7 September dress run completed this handoff with 24 images, six for each of four colours. Saved product configuration and storefront embed activation are separate states.
...
2. Use the listing run’s `colour-gallery-manifest.json`, with direct-to-draft authorization recorded and all selected assets internally accepted, uploaded and carrying verified Shopify MediaImage IDs. The compiler maps exact colour names to current option-value IDs. It also accepts the lower-level version-1 manifest below. Never guess colour from image position or translate arbitrary filenames.
...
The current workflow supplies `schema_version: 2`, `product_id`, exact `option_name`, `upload_approved: true` and `assets`. Record `upload_authorization: {"mode": "DIRECT_TO_DRAFT", "instruction": "the actual user instruction authorizing direct draft uploads"}`. This records standing upload permission, not human image review.

Each asset has the same `product_id`, exact `colour`, canonical `shot`, `shot_order` from 1 to 7, `qa_status: "accepted"`, `uploaded: true` and the verified full `shopify_media_id`. Keep `approval_status: "not_reviewed"` unless a human actually reviewed it. Preserve actual paths, dimensions, hashes, QA findings and any genuine earlier reviews in the run. Never invent QA or approval records.

Every selected colour must contain all seven unique shots. `side-movement` maps to `movement`; `ghost-flat` maps to `ghost`. Missing, duplicate, unchecked, unauthorized, unuploaded, foreign-product or unfinished media stops compilation. A direct-to-draft record cannot enable an ACTIVE or ARCHIVED product. The final human review occurs before activation.

Historical manifests without `upload_authorization` retain their actual `approval_status: "approved"` requirement. Version 1 retains six shots for those historical runs; do not downgrade a new seven-shot listing to that format.
