...
- **Metafields:** size · fabric · occasion · neckline · age group. Populate every applicable field established by the verified competitor PDP, including visible product-image facts; occasion follows the editorial rule below. Unknown physical facts stay blank and are flagged. An incomplete review DRAFT is not an activation-ready listing.
...
## Sizes — shared Ondine BODY guide

Ondine's default storefront guide is one shared BODY size guide at `profiles/ondine/size-guide.csv`. It retains the UK numeric labels from **UK 4 through UK 28**. Centimetres are authoritative; inches are derived from the corresponding centimetre value by dividing by 2.54.

This is a brand standard for customer guidance. It is never a product's measured garment dimensions, a verified statement of supplier equivalence, or a supplier-to-UK conversion table. Do not use it to relabel a source size, infer endpoints from a letter or range, or expand one source size into multiple UK variants.

The shared guide is the default for Ondine clothing through the existing Kiwi Size Chart app block. Shoes and other non-clothing categories need their own guide; this change does not migrate existing product-specific chart exceptions. A normal listing does not need a competitor size-chart extraction, a per-product Kiwi chart, or a product-specific Kiwi assignment. Do not flag a source size above UK 18: the shared guide covers UK 4–28.

Keep actual source option dimensions, ordered values and real combinations intact for variant identity. Keep the current UK-numeric default: an explicitly stated single UK numeric source label may retain that numeric identity when verified. Letter-only, other-market or ambiguous size labels remain unresolved under this default; the shared guide does not make them ready to list or authorize a conversion. Existing explicit product-specific label exceptions remain valid only for that product. New supplier-to-UK mapping is outside this change. Product-specific verified length, fit, measurements and the source-backed model fit note remain product evidence, separate from the shared BODY guide.
...
**Fit-note rule:** render `Model is [height] and wears UK [size]` (height always as `175cm`, never `1.75 m` or feet) as live selectable PDP text when the reference PDP explicitly provides both the model height and a verified UK size for that product. Do not use the shared BODY guide to convert a source model size. The note is product fit evidence and does not claim that the generated Ondine gallery model has those measurements. Missing height, missing worn size or an unverified UK conversion → omit the note and FLAG; never show an empty placeholder or carried-over number.
...
Resolve the most specific Shopify taxonomy category independently for each product from its verified product type and facts. Never hardcode `Dresses`, reuse a prior product's category, or assume that two products expose the same attribute set. Once the category is selected, query its available standardized attributes through the existing Shopify connector and fill every applicable one established by the verified competitor PDP, including its product images. Unknown, irrelevant or category-inapplicable attributes stay blank rather than receiving a guessed fallback.

Use Shopify taxonomy/metaobject references—not free-text substitutes. The mandatory sequence is: write the resolved category, query the category-constrained `shopify` metafield definitions available in the connected Ondine store, write each verified value, then read the resolved category and display values back through the connector. A category path in a plan, or a regular product view that omits category fields, does not pass this check. When the selected category combines concepts such as colour and pattern, preserve every verified part.
...
**Styling brief first (Ilias 2026-09-03):** before rendering any prompt, answer the five questions in `references/gallery-workflow.md` §7.0 (season on sale in the UK, occasion classification, UK buyer context, garment facts, competitor styling as evidence) in `styling_brief.json`, and let `scripts/apply_styling_brief.py` write footwear, accessories, lifestyle setting, movement, light and **one pose per model slot** into the facts file. **Every slide must communicate something different** (Ilias 2026-09-03): pack v4.5.0+ gives slots 01, 02, 03 and 05 their own `pose_01/02/03/05`, and the script refuses two poses whose wording overlaps by 45% or more. Never hand-patch a pose into a run prompt; fix the brief and re-run the script. The lifestyle setting follows the occasion, stays inside the warm-neutral palette, and never repeats any of the last three products (`<operator-state>/recent-settings.json`; the bundled empty file is only a starter).

**Multi-colour products (Ilias 2026-09-07):** internally check the full seven-shot lead-colour gallery, then adapt those same shots into seven images per additional selected colour. Change only garment/belt colour using actual source colour evidence; preserve model, pose, crop, construction, background and lighting. Reuse existing approved fronts. Internally check new colour fronts before generating their remaining views from the target front plus the matching approved lead-colour shot. Inspect all edits and upload accepted originals directly to the owned DRAFT without intermediate approval. Follow `references/gallery-workflow.md` §7.3b for the per-colour manifest, internal QA and standing upload authorization. Assign each colour front to that colour’s variants; keep the lead front featured.

**First-image QA:** internally validate slot 01 and its separate square, then continue without requesting first-image approval. Continue through internal QA and direct DRAFT upload under the shared gallery workflow; final human review occurs before activation. Before upload, read existing Shopify media. For new assets use `ondine-<product>-<colour>-<slot>` with stable IDs `01`, `01b`, `02`, `03`, `04`, `05`, `06`; preserve existing approved filenames and map them explicitly. Read back before any retry. The finished product must match the approved manifest: seven assets per colour in order, with no duplicates.

The live reference gallery is optional maintainer context. The bundled seven JSON templates and this product’s verified draft preview are the operator’s gallery standards.
