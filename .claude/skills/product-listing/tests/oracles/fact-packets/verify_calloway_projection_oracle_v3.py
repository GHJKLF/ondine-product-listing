...
MANIFEST = HERE / "nobodys-child-calloway.fact-packet-projection-v3.expected.json"
PRIOR_V2_MANIFEST = HERE / "nobodys-child-calloway.fact-packet-projection-v2.expected.json"
PRIOR_V2_REVIEWER_LOCK = HERE / "nobodys-child-calloway.fact-packet-projection-v2.atlas-lock.json"
...
def verify_pinned_inputs(manifest: dict[str, Any]) -> None:
    expected = {
        ...
        "source_capture_replay_implementation_lock": "3fa869ca05a3b9bd44337eea6b075452d1111f7e01fe2e6d179db6bcad64ec09",
        "ondine_profile": "e55ce269d910795d4ceff98e897ec5e879632bb4b2f9c41b4a2c863dcf07da59",
    }
...
def verify_records(manifest: dict[str, Any]) -> None:
    candidates = load(PRIOR_V2_MANIFEST)["bindings"]
...
def main() -> None:
    ...
    assert manifest["manifest_id"] == "nobodys-child-calloway-fact-packet-projection-oracle-v3"
    ...
    assert sha(PRIOR_V2_MANIFEST) == manifest["supersedes"]["manifest_sha256"]
    assert sha(PRIOR_V2_REVIEWER_LOCK) == manifest["supersedes"]["reviewer_lock_sha256"]
    ...
    assert author == {
        ...
        "attested_at": "2026-09-01T21:42:00+01:00",
    ...
    assert set(manifest["fixture"]) == {
        "fixture_id",
        "bundle",
        "bundle_hash_index",
        "source_oracle",
        "source_evidence_lock",
        "source_capture_replay_implementation_lock",
        "ondine_profile",
    }
    assert manifest["dependency_boundary"] == {
    ...
    assert "composition-contract.md" not in manifest_text
    assert "listing-plan.example.json" not in manifest_text
    ...
    v2_bindings = load(PRIOR_V2_MANIFEST)["bindings"]
    v3_bindings = manifest["bindings"]
    v2_canonical = json.dumps(v2_bindings, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
    v3_canonical = json.dumps(v3_bindings, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
    assert v2_bindings == v3_bindings
    assert v2_canonical == v3_canonical
    assert hashlib.sha256(v2_canonical).hexdigest() == hashlib.sha256(v3_canonical).hexdigest() == "7b89952844b9de29083fe5ec943adff7ed1fb23ab19a52a3987e0340bf95ce95"
    print(json.dumps({
        ...
        "v2_v3_bindings_canonical_bytes_equal": True,
        "bindings_canonical_sha256": "7b89952844b9de29083fe5ec943adff7ed1fb23ab19a52a3987e0340bf95ce95",
    }, indent=2))
