...
- **Front views require internal QA, not individual user approval.** Generate and validate the first portrait, its separate GMC square and the second-model view, then continue without asking Ilias to approve them individually. Ilias retains complete-gallery and Shopify-upload approval. For multiple colours, approve the full lead-colour gallery before adapting it, then review the complete additional-colour galleries before upload. Reference-pack assembly and template mapping are internal team QA and must not be pushed to Ilias for document review. Approval of one visual gate authorizes only that gate's stated next action.
...
Missing product facts stay missing; the absence of an exact camera view uses the Step 7 reference fallback and does not block gallery generation. Conflicting or ambiguous price, currency, option, media or physical facts → stop and explain the conflict. Preserve every source size value in the Ondine option set, including source sizes that are currently unavailable; source stock status is not copied.
...
For a complete listing run, read and follow [references/gallery-workflow.md](references/gallery-workflow.md) after the data-ready DRAFT passes. It starts with the styling brief (§7.0: season on sale, occasion, UK buyer context, garment facts, competitor styling as evidence → footwear, accessories, setting, movement, light), then internal reference/template QA, followed by internal QA of slot 01, its square and slot 01b, then the remaining Ilias gates: complete gallery and connector-only Shopify upload. Do not begin Step 7 when only a data-ready DRAFT was requested.
