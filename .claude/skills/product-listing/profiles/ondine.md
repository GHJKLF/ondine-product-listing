...
- **Reviewer-signed FactPacket gate:** normal Ondine composition accepts an `fp.*` binding only when it is present unchanged in a reviewer-signed, product-specific FactPacket projection manifest whose ID and SHA-256 are pinned by the ListingPlan. The manifest author and reviewer must be distinct identities. Missing, added, removed, reordered or changed bindings—including value, evidence, scope, conflict state, claim eligibility, policy eligibility or allowed transforms—are a STOP. The Calloway manifest is a golden-fixture dependency only, never a runtime default.
