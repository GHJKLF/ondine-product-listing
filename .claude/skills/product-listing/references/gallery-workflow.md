...
The lead portrait and separate GMC square no longer require individual user approval. Internal fidelity and framing QA still applies. Legacy template fields such as `requires_approved_slot_01` mean an internally QA-accepted continuity reference during generation; they do not impose a user approval pause. Final manifest approval remains actual gallery approval by Ilias. This overrides sample-first user-gate wording in older composition/template references. Additional-colour front review, full galleries and explicit upload approval remain unchanged. The second-model preview is now an internal QA step (Ilias, 2026-09-15).
